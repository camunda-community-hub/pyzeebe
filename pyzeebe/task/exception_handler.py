...
async def default_exception_handler(e: Exception, job: Job, job_controller: JobController) -> None:
    ...
    if isinstance(e, BusinessError):
        await job_controller.set_error_status(job, f"Failed job. Recoverable error: {e}", error_code=e.error_code)
    else:
        await job_controller.set_failure_status(job, f"Failed job. Error: {e}")
