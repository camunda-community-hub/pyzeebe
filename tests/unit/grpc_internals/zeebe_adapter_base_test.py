...
class TestShouldRetry:
    def test_returns_true_when_no_current_retries(self, zeebe_adapter: ZeebeGRPCAdapter):
        zeebe_adapter._max_connection_retries = 1
        assert zeebe_adapter._should_retry()

    def test_returns_false_when_current_retries_over_max(self, zeebe_adapter: ZeebeGRPCAdapter):
        zeebe_adapter._max_connection_retries = 1
...
class TestHandleRpcError:
    async def test_raises_internal_error_on_internal_error_status(self, zeebe_adapter: ZeebeGRPCAdapter):
    ...
    async def test_raises_back_pressure_error_on_resource_exhausted(self, zeebe_adapter: ZeebeGRPCAdapter):
    ...
    async def test_raises_deadline_exceeded_on_deadline_exceeded(self, zeebe_adapter: ZeebeGRPCAdapter):
    ...
    async def test_raises_gateway_unavailable_on_unavailable_status(
        self,
        zeebe_adapter: ZeebeGRPCAdapter,
    ):
    ...
    async def test_closes_after_retries_exceeded(self, zeebe_adapter: ZeebeGRPCAdapter):
        error = grpc.aio.AioRpcError(grpc.StatusCode.UNAVAILABLE, None, None)
    ...
    async def test_closes_after_internal_error(self, zeebe_adapter: ZeebeGRPCAdapter):
        error = grpc.aio.AioRpcError(grpc.StatusCode.INTERNAL, None, None)
